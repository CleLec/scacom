...
authornote: |
  The authors thank William Thorn (OECD) for helpful comments on an earlier version of this research.
  
abstract: |
  We investigated how literacy (reading competence) and numeracy (mathematical competence) develop during adulthood. We used data from two large-scale multi-wave  surveys (PIAAC-L, *N* = 1,775; NEPS, *N* = 3,087) that offer repeated measures of literacy and numeracy spaced three to six years apart. We examined mean-level change ($\Delta{T_1,T_2}$) and rank-order consistencies ($r_{T_1,T_2}$) in the population and in sociodemographic subgroups defined by age, gender, and education. We used plausible values (PVs) to account for measurement error. Results revealed that literacy and numeracy are highly but not perfectly rank-order stable ($.66 \leq r \leq .83$). Mean-level change was negligible for both skills and studies, but there were considerable individual differences in change. We found some evidence for moderate skill gains during young adulthood (18--29 years) and losses in old age (55+ years), resembling age profles from cross-sectional studies. However, subgroup diÃ¯Â¬â‚¬erences in change by age group, gender, and education were mostly small and did not replicate across studies. Our findings suggests that adultsÃ¢â‚¬â„¢ literacy and numeracy are not set in stone and can change even over relatively short time periods. They can serve as a benchmark against which to compare future longitudinal fndings on stability and change in literacy and numeracy.
