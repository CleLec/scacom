...
abstract: |
  We investigated individual differences in the development of literacy (reading competence) and numeracy (mathematical competence) during adulthood. Using data from two large-scale multi-wave studies (PIAAC-L, *N* = 1,775; NEPS, *N* = 3,087), we examined how mean-level change ($\Delta{T_1,T_2}$) and rank-order consistencies ($r_{T_1,T_2}$) of literacy and numeracy over three to six years are distributed in the total adult population and in sociodemographic subgroups defined by age, gender, and education. To account for measurement error, we employed plausible values (PV) methodology. Results revealed that literacy and numeracy are highly but not perfectly rank-order stable ($.66 \leq r \leq .83$). Mean-level change was negligible for both skills and studies, but there were considerable individual differences in change. We found some evidence for moderate skill gains during young adulthood (18--29 years) and losses in old age (55+ years), resembling age profiles from cross-sectional studies. However, subgroup differences in change by age group, gender, and education were mostly small and did not replicate across studies. Our findings suggests that adults' literacy and numeracy are not set in stone but can change even over relatively short time periods. They can serve as a benchmark against which to compare future longitudinal findings.
...
This pattern is reminiscent of the age trajectories of fluid (*Gf*, or the ability to process information, perceive and discriminate object relations, and resolve novel problems) and crystallized (*Gc*, or acquired knowledge and skills) intelligence hypothesized by @cattell1971 in his seminal work. According to his still influential theory, *Gf* starts to decline after the second decade of life, whereas *Gc* continues to increase well into adulthood and only declines in old age. Interestingly, the age trajectories of literacy and numeracy in the above studies neither follow Cattell's ideal-typical paths of *Gc* nor *Gf* but are best described as a mixture of both. This is plausible, considering that literacy and numeracy as conceptualized in international large-scale assessments (and in the present study) are conceptually closest to *Gc* but also require *Gf*, especially if tests are speeded (for details, see Measures). For comparison, longitudinal research on cognitive aging focusing on basic cognitive abilities found approximately linear declines in cognitive abilities (e.g., processing speed, reasoning, memory) from early adulthood onward that accelerated in old adulthood [@salthouse2019] -- with the exception of increases with age in vocabulary, which is closer to Cattell's conception of *Gc* than to *Gf* and an essential part of literacy skills.  
...
Our second measure of change was the rank-order consistency [sometimes called "differential stability"; e.g., @schalke2013]. Its measure is the Pearson correlation between the tests at two time points, $$r_{T_1,T_2} = \frac{\sum_{i=1}^{N} (y_{i,{T_1}} - \overline{y}_{T_1}) \times (y_{i,{T_2}} - \overline{y}_{T_2}) }{\sigma_{T_1}\sigma_{T_2}}$$ 
Here, $Y$ refers to the literacy or numeracy proficiency of an individual $i$ at time $T$. Rank-order consistency is a widely used standardized effect size measure that can be readily compared within and across studies.
...
Figure\ \@ref(fig:figure1) shows results for our first measure of change: mean-level change in literacy (blue) and numeracy (red), expressed in Cohen's $d_{av}$, across three years (PIAAC-L, left panel) to six (NEPS, right panel) years of adulthood in the total population and in the subgroups.

<!--\begin{landscape}-->

```{r figure1, echo = FALSE, warning = FALSE, fig.cap = "(ref:figure1-caption)", out.extra = ""}
