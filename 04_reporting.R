...
describe_data <- function(study) {
  data <- get(study) %>% 
    filter(.imp == 1)  %>% 
    mutate(edu0 = if_else(edugr == 0, 1, 0),
    ...
    select(age, age0, age1, age2, age3, female, edu0:edu2 #native, hhsize,
           #t1_pv, t2_pv
    ) %>%
...
data_descriptives <- cbind(describe_data("reading_piaac"), 
                           describe_data("reading_neps")[-1])

data_skills <- rbind(cbind(describe_skills("reading_piaac"), 
                           describe_skills("reading_neps"))[,-5],
                     cbind(describe_skills("math_piaac"), 
                           describe_skills("math_neps"))[,-5]) %>% 
  as.data.frame()
